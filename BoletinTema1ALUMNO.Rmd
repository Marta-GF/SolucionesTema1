...
#Ambas presentan un comportamiento muy similar. Sin embargo, no podemos afirmar que sigan una distribución gaussiana puesto que no son simétricas.
```
...
```{r}
#a)
#Para ello, debemos tener en cuentas que las 3 primeras etapas son: Importación, Preparación y transformación.
data(cabbages)
dim(cabbages)
str(cabbages)
head(cabbages)
...
un. Utiliza un procedimiento análogo para determinar si se aprecia una relación lineal entre los niveles de vitamina C, **VitC** en función del peso de la calabaza, **HeadWt**, en el conjunto de datos **cabbages**. b. Repite el apartado anterior, pero obteniendo un modelo para cada una de las dos variedades de calabaza, **Cult**. Ver[Parámetros básicos plot](https://www.statmethods.net/advgraphs/parameters.html).
